...
APP游客账号登录增加IP白名单控制。游客账号当前登录IP没有配置在账号IP白名单中时，登录失败；非游客账号不校验IP白名单。
...
2. 密码校验成功后判断账号类型。
3. 非游客账号不校验IP白名单。
4. 游客账号读取 `switch_set.id = 137`。
5. 开关状态为 `0` 时，使用 `account_type + 当前IP`、`account_id + 当前IP`、`account_type + account_id + 当前IP` 查询白名单。
6. 未命中白名单时返回登录失败，不创建token。
7. 开关状态为 `1` 或记录不存在时，不校验白名单。
...
3. 非游客账号不校验IP白名单。
4. 游客账号读取 `switch_set.id = 137`。
5. 开关状态为 `0` 时，校验 `account_type + 当前IP`、`account_id + 当前IP`、`account_type + account_id + 当前IP`。
6. 未命中白名单时返回登录失败，不创建token。
